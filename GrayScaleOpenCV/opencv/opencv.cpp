...
int main()
{
	...
	for (int y = 0; y < original_image.rows; y++) {
		for (int x = 0; x < original_image.cols; x++) {
			intensitiy = (0.114 * original_image.at<Vec3b>(y, x)[0]
				+ 0.587 * original_image.at<Vec3b>(y, x)[1]
				+ 0.229 * original_image.at<Vec3b>(y, x)[2]);

			serial_image.at<uchar>(y, x) = intensitiy;
		}
	}
	timer.offTimer(0);
	...
	/* parllel code*/
	timer.onTimer(1);
	#pragma omp parallel for private(intensitiy)
	for (int y = 0; y < original_image.rows; y++) {
		for (int x = 0; x < original_image.cols; x++) {
			intensitiy = (0.114 * original_image.at<Vec3b>(y, x)[0]
				+ 0.587 * original_image.at<Vec3b>(y, x)[1]
			...
			parallel_image.at<uchar>(y, x) = intensitiy;
		}
	}
	timer.offTimer(1);
